...
const ISACanons = require('metadata/lib/ISAlgorithm').canons;
const promiseISAlgorithm = require('../lib/promiseISAlgorithm');
const should = require('should');
const testUtils = require('node-utils/testing-utils/lib/utils');
const util = require('util');

describe('Promise Identity Syndication Algorithm Tests', function () {
  'use strict';
  ...
  it('1.1 should post a request to the metadata service and return a structure with results', function () {
    ...
    nock(API_GATEWAY_URL)
          ...
          .reply(function () { // not used uri, requestBody) {
            return [
              HttpStatus.OK,
              createMDJWT(ISAlgorithm),
              { 'content-type': 'text/plain', },
            ];
          });

    return promiseISAlgorithm.execute(dummyServiceCtx, ISAlgorithm['@id'], 'msg-1', 'actionMsg-1')
      .then(function (result) {
        result.should.have.property('isa');
      },
  ...
  function createISAlgorithm() {
    ...
    return ISACanons.createISAlgorithm(props);
  }
