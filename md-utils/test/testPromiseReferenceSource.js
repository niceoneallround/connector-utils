...
const RSCanons = require('metadata/lib/ReferenceSource').canons;
const promiseReferenceSource = require('../lib/promiseReferenceSource');
const should = require('should');
const testUtils = require('node-utils/testing-utils/lib/utils');
const util = require('util');

describe('Promise Reference Source Tests', function () {
  'use strict';
  ...
  it('1.1 should post a request to the metadata service and return a structure with results', function () {
    ...
    let referenceSource = createReferenceSource();
    let pathUrl = '/v1/metadata/reference_source___io___webshield___test--test-rs-1';
    nock(API_GATEWAY_URL)
          ...
          .reply(function () { // not used uri, requestBody) {
            return [
              HttpStatus.OK,
              createMDJWT(referenceSource),
              { 'content-type': 'text/plain', },
            ];
          });

    return promiseReferenceSource.execute(dummyServiceCtx, referenceSource['@id'], 'msg-1', 'actionMsg-1')
      .then(function (result) {
        result.should.have.property('rs');
      },
  ...
  function createReferenceSource() {
    ...
    return RSCanons.createTestReferenceSource(props);
  }
